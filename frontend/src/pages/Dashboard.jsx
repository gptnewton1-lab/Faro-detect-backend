...
export default function Dashboard() {
  ...
  const getStatusColor = (status) => {
    if (status === 'Safe' || status === 'SAFE') return 'bg-green-500';
    if (status === 'Warning' || status === 'SUSPICIOUS') return 'bg-yellow-500';
    return 'bg-red-500';
  };
      ...
      <div className="max-w-3xl mx-auto bg-gray-800 p-6 rounded-2xl shadow-xl">
        ...
        {result && (
          <div className="mt-6 p-4 bg-gray-700 rounded-lg">
            ...
            <p>Status: <span className={`inline-block px-3 py-1 rounded-full text-xs font-bold ${getStatusColor(result.status)}`}>{result.status}</span></p>
        ...
        <div className="mt-3 space-y-2 max-h-60 overflow-y-auto">
          ...
          {history.map((item, i) => (
            <div key={i} className="bg-gray-700 p-3 rounded-lg flex justify-between">
              ...
              <span className={`text-xs px-2 py-1 rounded-full ${item.status === 'SAFE' ? 'bg-green-600' : 'bg-red-600'}`}>
                {item.status}
              </span>
            </div>
          ))}
        ...
        <p className="mt-8 text-center text-xs text-gray-600 border-t border-gray-700 pt-4">
          Presidential ICT Prize 2026
        </p>
      </div>
    </div>
  );
