...
export default function Login() {
  ...
  const handleLogin = async (e) => {
    ...
    try {
      const formBody = new URLSearchParams(); // FastAPI's OAuth2PasswordRequestForm expects form-encoded data, not JSON
      formBody.append('username', email); // The backend reads the email from the "username" field by OAuth2 convention
      formBody.append('password', password); // Password goes in as-is
      const res = await fetch(`${API_URL}/auth/login`, {
        method: 'POST',
        headers: { 'Content-Type': 'application/x-www-form-urlencoded' }, // Must match the encoding used below
        body: formBody
      });
